...
| `0.3.6-jt.6`: `rr mkdir` writes a folder through the same sync v3 endpoints as `push`: one `.metadata` file in the shape the reMarkable software writes, one new root-index line, `--dry-run` first; it refuses a name already in use, a parent that is missing, not a folder or in the trash, and a listing it could not read in full; a push and `mkdir` share one write loop | `rr mkdir` called the document API, which refuses a browser-paired device with HTTP 401 even with Connect, so the client could not create the folder it pushes into | Jeremiah Tims |
